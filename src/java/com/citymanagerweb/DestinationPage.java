...
public class DestinationPage extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet DestinationPage</title>");
            out.println("</head>");
            out.println("<body>");
            final HttpSession session = request.getSession();
            if (session != null && !session.isNew() && session.getAttribute("userName") != null) {
                out.println("<span>Welcome</span>&nbsp;<strong> " + session.getAttribute("userName") + "</strong>");
            } else {
                out.println("<span>Unauthorized</span>");
            }
            out.println("</body>");
            out.println("</html>");
        }
